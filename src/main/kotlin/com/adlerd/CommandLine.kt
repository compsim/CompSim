...
class CommandLine(val machine: Machine) {
    ...
    init {
        ...
        this.commandsSet = TreeSet(object : Comparator<Command> {
            override fun compare(command1: Command, command2: Command): Int {
                val command1str =
                    command1.usage.split("\\s+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]
                val command2str =
                    command2.usage.split("\\s+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]
                return command1str.compareTo(command2str)
            }

            override fun equals(other: Any?): Boolean {
                val otherObject =
                    (other as Command).usage.split("\\s+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]
                val command =
                    (this as Command).usage.split("\\s+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]
    ...
    fun getNextCommand(): String {
        return this.commandQueue.removeFirst() as String
    }

    fun hasQueuedStop(): Boolean {
        return (this.commandQueue.first as String).equals("stop", ignoreCase = true)
    }
...
//    }
    ...
    /**
     *  Function which holds all commands
     */
                    ...
                    if (startAddress == Int.MAX_VALUE) {
                        ...
                        if (endAddress == Int.MAX_VALUE) {
                            ...
                            if (endAddress < startAddress) {
                                ...
                                writer.close()
                                return "com.adlerd.Memory dumped."
                            }
                        } else {
        ...
        this.commands["counters"] = object : Command {
            ...
            override fun doCommand(argArray: Array<String>, argSize: Int): String {
                return if (argSize != 1) {
                    ...
                    output += "com.adlerd.Instruction count: ${this@CommandLine.machine.instructionCount}\n"
                    ...
                    if (argSize == 2) {
                        ...
                        if (symbol == Integer.MAX_VALUE) {
                            ...
                            if (startAddr == Integer.MAX_VALUE) {
                                ...
                                return if (argSize == 3) "com.adlerd.Memory location ${Word.toHex(
                                    startAddr
                                )} updated to ${argArray[argSize - 1]}" else "com.adlerd.Memory locations ${Word.toHex(
                                    endAddr
    ...
    fun setRegister(register: String, regValue: Int): String? {
        var update = "Register ${register.toUpperCase()} updated to regValue ${Word.toHex(regValue)}"
        when {
