...
class Word {
    ...
    private fun setField(var1: Int, var2: Int, var3: Int) {
        if (var3 > var2) {
            throw AsException("Hi and lo bit operands reversed.")
    ...
    @Throws(AsException::class)
    fun setUnsignedField(var1: Int, var2: Int, var3: Int) {
        if (var3 > var2) {
            throw AsException("Hi and lo bit operands reversed.")
        } else if (var2 in 0..15 && var3 <= 15 && var3 >= 0) {
            val var4 = var1 shr var2 - var3 + 1
            if (var4 == 0) {
                this.setField(var1, var2, var3)
            } else {
                throw AsException("Immediate out of range: $var1")
    ...
    companion object {
        ...
        fun parseNum(num: String): Int {
            val convertedNum: Int = try {
                if (num.startsWith('x')) {
                    Integer.parseInt(num.replace('x', '0'), 16)
                } else if (num.startsWith('b')) {
                    Integer.parseInt(num.replace('b', '0'), 2)
                } else {
